    ...
    <div className="column is-one-third" style={props.style}>
      <div className="card has-background-light">
        <header className="card-header">
          ...
          <button
            className="card-header-icon py-1 px-3"
            aria-label="complete task"
            title="Complete"
            onClick={() => completeTask(props.task)}
          >
            {!props.task.status ? (
        ...
        <div className="p-4">
          <div className="content">{props.task.description}</div>
        </div>
        <footer className="card-footer is-flex is-justify-content-space-between">
          <span className="p-3">
            {new Date(props.task.datetime).toLocaleString()}
          </span>
          <span
            className={`p-3 has-text-primary has-text-weight-bold 
